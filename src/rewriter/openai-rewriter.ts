...
const log = createScopedLogger('OpenAIRewriter');
...
function generateTags(article: RawArticle, categories: string[]): string[] {
  ...
  // Always add niche-specific tags based on content
  const lowerTitle = article.title.toLowerCase();
  const lowerDesc = article.description.toLowerCase();
  const allText = lowerTitle + ' ' + lowerDesc;

  if (allText.includes('openai') || allText.includes('gpt') || allText.includes('chatgpt') || allText.includes('o1') || allText.includes('o3') || allText.includes('sora')) {
    tags.add('openai');
  }
  if (allText.includes('anthropic') || allText.includes('claude')) {
    tags.add('anthropic');
    tags.add('claude-code');
  }
  if (allText.includes('google') || allText.includes('gemini')) {
    tags.add('google');
    tags.add('gemini');
  }
  if (allText.includes('meta') || allText.includes('llama')) {
    tags.add('meta');
    tags.add('llama');
  }
  if (allText.includes('no-code') || allText.includes('nocode') || allText.includes('low-code') || allText.includes('lowcode')) {
    tags.add('no-code');
  }
  if (allText.includes('vibe') || allText.includes('prompt') || allText.includes('cursor') || allText.includes('copilot')) {
    tags.add('vibe-coding');
  }
  if (allText.includes('llm') || allText.includes('large language model') || allText.includes('foundation model')) {
    tags.add('llm');
  }
  if (allText.includes('ai') || allText.includes('artificial intelligence') || allText.includes('machine learning') || allText.includes('deep learning')) {
    tags.add('inteligencia-artificial');
...
export async function rewriteArticle(
  ...
  const audiencePrompt = audienceMode === 'technical'
    ? 'assume o leitor tem conhecimento técnico avançado em programação e IA'
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Você é um editor-chefe de um blog de tecnologia especializado em Inteligência Artificial, No-Code, LLMs, Vibe Coding e ferramentas como OpenAI, Claude, Gemini.
...
- Se a notícia mencionar algo técnico (modelos, benchmarks, preços, datas), mantenha esses dados precisos`,
        },
      ],
      temperature: 0.7,
      max_tokens: 4000,
    ...
    log.error('Failed to rewrite article with OpenAI', { error: String(error) });
    return null;
  }
}
